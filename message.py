# Karol Lebiedziński, 278819
# przyjąłem na sztywno, że celem ukrywania wiadomości jest przesyłanie tajnego klucza długości 64bity
import sys
import re
...
def hide_message_algorithm2(cover, message):
    ...
    # if len(set(cover.split())) < len(message):
    # print(unique_spaces)
    ...
    for line in watermark:
        ...
        for char in line:
            if len(messageBinary) == 0:
                break
            if char == " ":
                print("wchodze w spacje")
                print(line[line.index(char)])
                print(line[line.index(char) + 1])
                print(line[line.index(char) - 1])
                if (
                    line[line.index(char) + 1] == " "
                    or line[line.index(char) - 1] == " "
                ):
                    print("siema to jest tab jakis kontynuuje sb")
                    continue
                elif messageBinary[0] == "1":
                    line.replace(" ", "  ", 1)
                    print("zamienilem na podwojna")
                messageBinary = messageBinary[1:]
                print(messageBinary[1])
    watermark = "\n".join(watermark)
    print(messageBinary)
    return watermark
